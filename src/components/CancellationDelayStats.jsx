...
// Per-line cancellation + delay analytics, rendered on the line/route page from
// the stats computed by computeCancellationDelayStats. Three blocks, each shown
// only when it has data:
//   1. A stat grid (counts, per-week rates, recency, typical delay length).
//   2. Most-cancelled departures, grouped by origin station.
//   3. When cancellations happen, by part of day.
// Cancellations only populate for rail (single-departure announcements); on a
// bus route the section degrades to just the delay-alert stats. Purely
// descriptive — counts and bars, no verdict.
export default function CancellationDelayStats({ stats }) {
  ...
  if (d.medianDurationMin != null) {
    cells.push({ v: formatMinutesAsHours(d.medianDurationMin), l: 'typical delay length' });
  }
        ...
        {topOrigins.length > 0 && (
          <div className="bg-white dark:bg-gh-surface rounded-lg border border-slate-200 dark:border-gh-border p-4">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-slate-100 dark:border-gh-border">
              Grouped by the station the cancelled departure originates from.
            </p>
          </div>
        )}
