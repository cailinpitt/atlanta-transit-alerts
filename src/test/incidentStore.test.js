...
// Build a minimal incident. The store only touches id/mode/routes for the
// website gate + id resolution; lifecycle is enough for the rest. mode 'rail'
// (or 'bus') passes isWebsiteIncident; a non-website mode is filtered out.
function incident(id, { mode = 'rail', routes = ['red'] } = {}) {
  return {
    id,
    mode,
    routes,
...
describe('atlantaMonthKey', () => {
  it('honors the Atlanta timezone boundary', () => {
    // 2026-06-01T03:00Z is still 2026-05-31 23:00 in Atlanta (EDT, -4).
    expect(atlantaMonthKey(Date.parse('2026-06-01T03:00:00Z'))).toBe('2026-05');
    expect(atlantaMonthKey(Date.parse('2026-06-01T05:00:00Z'))).toBe('2026-06');
  });
});

describe('loadRecent', () => {
  it('fetches the recent file with revalidation and applies the website gate', async () => {
    const calls = mockFiles({
      'alerts-recent.json': {
        generated_at: 5,
        incidents: [incident('a'), incident('drop', { mode: 'general' })],
      },
    });
    const out = await loadRecent();
    expect(out.generated_at).toBe(5);
    // The non-website incident is filtered out at the boundary.
    expect(out.incidents.map((i) => i.id)).toEqual(['a']);
    expect(calls[0]).toEqual({ path: 'alerts-recent.json', cache: 'no-cache' });
  });
});

describe('loadMonth', () => {
  it('default-caches + memoizes a closed month (one fetch for repeat calls)', async () => {
    const calls = mockFiles({
  ...
  it('revalidates the current month and does not memoize it', async () => {
    const key = atlantaMonthKey(Date.now());
    const calls = mockFiles({
...
describe('loadLine', () => {
  it('encodes the key, memoizes, and gates', async () => {
    const calls = mockFiles({
      'incidents/by-line/110%20Peachtree.json': {
        line: '110 Peachtree',
        incidents: [incident('x', { mode: 'bus', routes: ['110 Peachtree'] })],
      },
    });
    await loadLine('110 Peachtree');
    await loadLine('110 Peachtree');
