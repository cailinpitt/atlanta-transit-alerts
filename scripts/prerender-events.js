...
async function renderPng(page, html, outPath) {
  ...
  // JPEG (not PNG): the per-event cards are gradient+text, and the PNGs
  // (~440KB each) bloat the Pages artifact toward the 1GB limit as events
  // accumulate. JPEG q82 is ~6x smaller with no visible loss on these cards.
...
async function main() {
  if (process.env.SKIP_OG_RENDER === '1') {
    console.log('prerender-events: SKIP_OG_RENDER=1 — skipping event OG render');
    return;
  }
